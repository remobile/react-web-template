...
export default class PickerAndroid extends Component {
    ...
    _move (dy) {
        ...
        this.middleHeight = Math.abs(-index * rowHeight + dy);
        this.up && this.up.setNativeProps({
            style: {
                marginTop: (3 - index) * rowHeight + dy,
            },
        });
        this.middle && this.middle.setNativeProps({
            style: {
                marginTop: -index * rowHeight + dy,
            },
        });
        this.down && this.down.setNativeProps({
            style: {
                marginTop: (-index - 1) * rowHeight + dy,
            },
        });
    }
    ...
    _handlePanResponderMove (evt, gestureState) {
		...
		// turn down
        if (dy > 0) {
            this._move(dy > this.index * rowHeight ? this.index * rowHeight : dy);
        } else {
            this._move(dy < (this.index - this.state.items.length + 1) * rowHeight ? (this.index - this.state.items.length + 1) * rowHeight : dy);
        }
    }

    _handlePanResponderRelease (evt, gestureState) {
        let middleHeight = this.middleHeight;
        this.index = middleHeight % rowHeight >= 20 ? Math.ceil(middleHeight / rowHeight) : Math.floor(middleHeight / rowHeight);
        this._move(0);
        this._onValueChange();
    }
    ...
    render () {
        ...
        let upViewStyle = {
            marginTop: (3 - index) * rowHeight,
            height: length * rowHeight,
        };
        let middleViewStyle = {
            marginTop:  -index * rowHeight,
        };
        let downViewStyle = {
            marginTop: (-index - 1) * rowHeight,
            height:  length * rowHeight,
        };

        return (
			// total to be 90*2+rowHeight=220 height
