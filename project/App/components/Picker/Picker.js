...
import Picker from './SinglePicker';
...
export default class PickerAny extends Component {
    ...
    _renderParallelWheel (pickerData) {
        return pickerData.map((item, index) => {
            return (
                <View style={[styles.pickerWheel, { width: 125 }]} key={index}>
                    <Picker
    ...
    _renderCascadeWheel (pickerData) {
        const _width = this.state.style.width || width;
        let thirdWheel = this.state.thirdWheelData && (
        <View style={[styles.pickerWheel, { width: _width / 3 }]}>
            <Picker
                ref={'thirdWheel'}
        ...
        return (
            <View style={[styles.pickerWrap, { width: _width }]}>
                <View style={[styles.pickerWheel, { width: this.state.thirdWheelData ? _width / 3 : _width / 2 }]}>
                    <Picker
                        ref={'firstWheel'}
    ...
    render () {
        let mask = this.state.showMask ? (
            <View style={styles.mask} >
                <Text style={{ width: width, height: height }} onPress={this._pickerCancel.bind(this)} />
            </View>
						) : null;
